...
def gen_dadda_config(count_list):
    ...
    count_list_remaining_now = []
    count_list_remaining_next = count_list[:]
    ...
    j = 0
    FAreplace_index = len(count_list_remaining_next)
    FAreplace_lastIndex = len(count_list_remaining_next)
    FAreplace_FLAG = 0
    FAreplace_lastFLAG = 0
    while(max(count_list_remaining_next) > 2):
        count_list_remaining_now = count_list_remaining_next[:]  # don't omit [:]!!!!!!!
        FA_START_FLAG = 0
        # if the last FA in each round can be replaced by HA, mark it in advance
        FAreplace_lastIndex = FAreplace_index
        FAreplace_lastFLAG = FAreplace_FLAG
        for i in range( len(count_list_remaining_now)-1, -1, -1 ):
            count = count_list_remaining_now[i]
            count_next = count_list_remaining_now[i-1]
            count_next_next = count_list_remaining_now[i-2]
            if count > 2 and count_next <= 2 and count_next_next <= 2:
                if FAreplace_lastFLAG == 0 and i < FAreplace_lastIndex and FAreplace_lastIndex != len(count_list_remaining_now):
                    FAreplace_index = i
                    FAreplace_FLAG = 1
                elif FAreplace_lastFLAG == 0 and i < FAreplace_lastIndex and FAreplace_lastIndex == len(count_list_remaining_now):
                    FAreplace_index = i
                elif FAreplace_lastFLAG == 0 and i >= FAreplace_lastIndex:
                    FAreplace_index = i
                elif FAreplace_lastFLAG == 1:
                    FAreplace_index = i
                break
            else:
                FAreplace_index = i
                if count > 2 and (count_next > 2 or count_next_next > 2):
                    break
        # print(FAreplace_index, FAreplace_FLAG)

        for i in range( len(count_list_remaining_now) ):
            element = []
            count = count_list_remaining_now[i]
            if i == FAreplace_index and FAreplace_FLAG == 1:
                element.append(i)
                ...
                count_list_remaining_next[i] -= 1
                count_list_remaining_next[i+1] += 1
            ...
            if count > 2 and FA_START_FLAG == 0:
                ...
                # 1FA
                if i != len(count_list_remaining_now)-1:
                    element.append(i)
                    element.append(count_list_finishnum[i])
                    element.append(count_list_finishnum[i]+1)
            ...
            if len(element) > 1:
                dadda_config_list.append( tuple(element) )
        j += 1
    return dadda_config_list, count_list_maxindex, count_list_remaining_next
