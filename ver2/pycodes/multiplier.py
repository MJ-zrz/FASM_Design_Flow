...
appLut_inst_path_dict = {
    "LUT_a": [
        ...
        "../inst_src/lut/LUT_dse1_inst.v",
        "../inst_src/lut/LUT_dse2_inst.v",
        "../inst_src/lut/LUT_dse3_inst.v",
        "../inst_src/lut/LUT_dse4_inst.v",
        "../inst_src/lut/LUT_dse5_inst.v",
        "../inst_src/lut/LUT_dse6_inst.v",
        "../inst_src/lut/LUT_dse7_inst.v",
        "../inst_src/lut/LUT_dse8_inst.v",
        "../inst_src/lut/LUT_dse9_inst.v"
    ],
    "LUT_b": [
...
class Multiplier:
    ...
    def gen_2inputAdderFiles(self):
        addermod_path = "../mod_src/adder/"
        modfilename = "adder_mod.v"
        inst_num = math.ceil( (self.MultWidth_f + self.MultWidth_b) / 4 )
        self.CLAadderWidth = 4 * inst_num
        targetfilename = f"adder{self.CLAadderWidth}.v"
        copyfile(addermod_path + modfilename, self.output_directory, targetfilename)
        ...
        adder4_instantion = ""
        adder4_inst_initialinfo = '''adder4 adder4_inst<#width1>(
        .A              (A[<#width3>:<#width2>]     )       ,
        .B              (B[<#width3>:<#width2>]     )       ,
        .c0             (c<#width4>         )       ,
        .S              (S[<#width3>:<#width2>]     )       ,
        ...
        for i in range(inst_num):
            adder4_inst_info = re.sub("(<#width1>)", f"{i}", adder4_inst_initialinfo)
            adder4_inst_info = re.sub("(<#width2>)", f"{4*i+1}", adder4_inst_info)
            adder4_inst_info = re.sub("(<#width3>)", f"{4*(i+1)}", adder4_inst_info)
            adder4_inst_info = re.sub("(<#width4>)", f"{4*i}", adder4_inst_info)
            adder4_instantion += adder4_inst_info
            if (i!=0):
                wire_definition += f"c{4*i}, "
        wire_definition = wire_definition[:-2] + ";"
        content = ""
        with open(self.output_directory + targetfilename, "r", encoding="utf-8") as f:
            content = f.read()
            content = re.sub("(<adder4 instantiation>)", adder4_instantion, content)
            content = re.sub("(<wire definition>)", wire_definition, content)
            content = re.sub("(<#width1>)", f"{self.CLAadderWidth}", content)
        with open(self.output_directory + targetfilename, "w", encoding="utf-8") as f:
            f.write(content)
        
        adderfile_path = "../adder_src/"
